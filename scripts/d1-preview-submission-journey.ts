...
async function d1(
  ...
  const response = await fetch(
    `https://api.cloudflare.com/client/v4/accounts/${required('CLOUDFLARE_ACCOUNT_ID')}/d1/database/${databaseId}/query`,
    {
      ...
      body: JSON.stringify({
        batch: plans.map((plan) => ({ sql: plan.sql, params: plan.params })),
      }),
    },
  );
...
function measuredPrivateCounts(
...
async function post(url: URL, body: unknown): Promise<Record<string, unknown>> {
  url.searchParams.set('replatformDiagnostic', 'a4f2');
  const response = await fetch(url, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'x-replatform-preview-intake': required(
  ...
  if (!response.ok)
    throw new Error(
      `Preview submission request failed (${response.status}): ${String(payload.diagnostic ?? payload.code ?? 'unknown')}`,
    );
  return payload;
}
async function exerciseRateLimit(
  ...
  throw new Error(
    'Preview Submission rate limit did not fail closed with 429.',
  );
}
async function createAndVerify(base: URL, siteId: string, suffix: string) {
  ...
  if (verified.status !== 'verified')
    throw new Error(
      'Real Preview badge verification did not reach verified state.',
    );
...
async function main(): Promise<void> {
  ...
  if (!siteValue || !output || !journalPath)
    throw new Error(
      'Usage: d1-preview-submission-journey.ts <site> <output.json> <journal.json>',
    );
  ...
  if (Object.values(targetBefore).some((value) => Number(value) !== 0))
    throw new Error(
      'Controlled Preview target contains copied private state before journeys.',
    );
  const rateBefore = new Set(
    (
      (
        await d1(targetId, [
          plan('SELECT fingerprint_hash FROM listing_submission_rate_limits'),
        ])
      )[0]?.results ?? []
  ...
  const publication = (
    await d1(targetId, [
      plan('SELECT * FROM publication_state WHERE site_id=?', [siteId]),
    ])
  )[0]?.results?.[0];
  if (
    !publication ||
  ...
  const publicationRunIds =
    (
      await d1(targetId, [plan('SELECT id FROM publication_runs ORDER BY id')])
    )[0]?.results?.map((row) => String(row.id)) ?? [];
  ...
  if (sourceSnapshot.checksum !== targetSnapshot.checksum)
    throw new Error(
      'Pre-journey source and target application snapshots differ.',
    );
  const journal = {
    version: 1,
    siteId,
  ...
  if (
    !previewResponse.ok ||
    !(await previewResponse.text()).includes('Preview Journey')
  )
    throw new Error('Private Preview journey failed.');
  const selected = (
    await d1(targetId, [selectSubmissionForDecisionPlan(approved.id, siteId)])
  )[0]?.results?.[0];
  ...
  const approvedRow = (
    await d1(targetId, [
      plan('SELECT status FROM listing_submissions WHERE id=?', [approved.id]),
    ])
  )[0]?.results?.[0];
  if (approvedRow?.status !== 'approved')
    throw new Error('Preview approval journey failed.');
  ...
  await d1(targetId, [
    plan(
      "UPDATE listing_submissions SET listing_id=NULL,status='verified' WHERE id=?",
      [approved.id],
    ),
    ...
    plan('DELETE FROM listing_submission_notifications WHERE submission_id=?', [
      approved.id,
    ]),
    plan('DELETE FROM listing_submission_events WHERE submission_id=?', [
      approved.id,
    ]),
    plan(
      'DELETE FROM listing_submission_resource_links WHERE submission_id=?',
      [approved.id],
    ),
    plan('DELETE FROM listing_submission_faqs WHERE submission_id=?', [
      approved.id,
    ]),
  ...
  const rejectedRow = (
    await d1(targetId, [
      plan('SELECT status FROM listing_submissions WHERE id=?', [rejected.id]),
    ])
  )[0]?.results?.[0];
  if (rejectedRow?.status !== 'rejected')
    throw new Error('Preview rejection journey failed.');
  ...
  await d1(targetId, [
    plan(
      'DELETE FROM listing_submission_rate_limits WHERE fingerprint_hash=?',
  ...
  const generated = {
    ...
    notificationSecretRows: Math.max(
      0,
      Number(targetPeak?.notificationSecretRows) -
        Number(targetBefore?.notificationSecretRows),
    ),
