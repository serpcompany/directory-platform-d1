...
export function validateWorkerConfig(environment: WorkerEnvironment): void {
  ...
  if (config.main !== 'apps/serp.software/.open-next/worker.js')
    throw new Error('Unexpected Worker entrypoint.')
  if (
    config.assets?.directory !== 'apps/serp.software/.open-next/assets' ||
  ...
  if (
    ...
    !config.routes?.some(
      route => route.pattern === 'serp.software/*' && route.zone_name === 'serp.software'
    )
  ) {
  ...
  if (
    config.vars?.SITE_ID !== 'serp.software' ||
    config.vars.NEXT_PUBLIC_SITE_ID !== 'serp.software'
  )
    throw new Error('Worker templates must remain scoped to serp.software.')
  ...
  if (source.toLowerCase().includes(otherEnvironment))
    throw new Error(`${configPaths[environment]} references ${otherEnvironment}.`)
}

function assertProductionWorkflow(
  ...
  if (
    ...
    throw new Error(
      'Production execution is authorized only by a protected production workflow in GitHub Actions.'
    )
  }
  if (env.GITHUB_REF !== 'refs/heads/main' || !env.GITHUB_SHA)
    throw new Error('Production execution requires the main branch and a nonempty GitHub SHA.')
  const expectedConfirmation = isPublicationWorkflow
    ? 'publish-serp.software-production'
    : isSubmissionApprovalWorkflow
      ? 'approve-serp.software-submission-production'
      : 'deploy-serp.software-production'
  if (env.WORKER_PRODUCTION_CONFIRM !== expectedConfirmation)
    throw new Error('Explicit production confirmation is required.')
  const status = dependencies.run('git', ['status', '--porcelain', '--untracked-files=normal'], {
    encoding: 'utf8'
  })
  if (status.status !== 0 || String(status.stdout ?? '').trim())
    throw new Error('Production execution requires a clean source checkout.')
...
function runProduction(
  ...
  if (command === 'import') {
    ...
    if (stateRows.length > 0)
      throw new Error('Refusing to overwrite an existing production publication.')
  ...
  console.log(
    JSON.stringify({ checksum: row.checksum, count: row.approved_count, version: row.version })
  )
}
...
function printRemotePlan(
  ...
  if (env.D1_REMOTE_CONFIRM !== `plan-serp.software-${environment}`)
    throw new Error(`D1_REMOTE_CONFIRM must equal plan-serp.software-${environment}.`)
  validateWorkerConfig(environment)
  ...
  console.log(
    JSON.stringify(
      { dryRun: true, environment, kind, migrations, checksum, executesRemoteCommand: false },
      null,
      2
    )
  )
...
export function runWorkerRelease(
  ...
  if (command === 'plan-migration' || command === 'plan-verify') {
    printRemotePlan(command === 'plan-migration' ? 'migration' : 'verify', environment, env)
    return
  }
  if (
    environment === 'production' &&
    ['backup', 'deploy', 'import', 'migrate', 'upload', 'verify'].includes(command)
  ) {
    runProduction(command as ProductionCommand, env, dependencies)
    return
  }
