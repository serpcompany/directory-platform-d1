...
describe('category-navigation', () => {
  ...
  it('matches category pages against either the primary or a secondary category', () => {
    expect(listingMatchesCategory(sampleListings[0]!, 'developer-tools')).toBe(true)
    expect(listingMatchesCategory(sampleListings[0]!, 'video-downloaders')).toBe(true)
    expect(listingMatchesCategory(sampleListings[0]!, 'infrastructure-cloud')).toBe(false)
  })
  ...
  it('uses the requested site category set when a site id is provided', () => {
    expect(
      getUnknownCategorySlugs(
        [{ category: 'video-downloaders' }, { category: 'developer-tools' }],
        'serp.software'
      )
