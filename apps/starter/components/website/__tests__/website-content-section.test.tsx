...
describe('WebsiteContentSection', () => {
  it('renders the provided long-form content when it exists', () => {
    ...
    expect(document.querySelector('.prose')).toHaveTextContent(
  ...
  it('renders the first media image as the featured image when available', () => {
    ...
    expect(screen.getByRole('img', { name: 'Example Project featured image' })).toHaveAttribute(
      'src',
      'https://cdn.example.com/featured-image.png'
    )
  })
...
- Product page: https://example.com`,
          resourceLinks: [{ label: 'Help Center', url: 'https://example.com/help' }]
        }}
      />
    )
    ...
    expect(document.querySelector('.prose')).not.toHaveTextContent('## Links')
    expect(document.querySelector('.prose')).not.toHaveTextContent('Product page:')
  })
  ...
  it('shows all assigned categories when a listing belongs to more than one category', () => {
    ...
    expect(screen.getByText('Developer Tools, Video Downloaders')).toBeInTheDocument()
  })
})
